...
def loadcurry(plan, name, currypath=None, **kwds):
  ...
  if os.path.isdir(filename):
    ...
    return loadpython(plan.interp, filename)
  elif filename.endswith('.so'):
    return loadso(plan.interp, filename)
  else:
...
def loadpython(interp, pyfile):
  '''
  Reads a Python file generated with ``save`` and returns the IModule it
  defines.
  '''
  assert os.path.exists(pyfile)
  assert pyfile.endswith('.py')
  return interp.load(pyfile)

def loadso(interp, sofile):
  '''
  Loads a shared object generated by Sprite and returns the IModule it defines.
  '''
  assert os.path.exists(sofile)
  assert sofile.endswith('.so')
  from ..backends.cxx import cyrtbindings as cyrt
