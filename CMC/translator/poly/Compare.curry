...
-- For a type t, the new function is the generic comparison equality of t.
-- The name of the new function is modname.compare.t 
-- For a polymorphic type t x, the equality of x is Prelude.compare

import ICurry
import Fix

-- the name of the comparison function for a given typename
xcompare typename = "compare."++typename
primitive typename = "primitive."++xcompare typename
...
make_funct modname ((_,typename),[])
  ...
  where qname = (modname, xcompare typename)
...
make_entry q r 
  ...
  | otherwise = (r, [Return (Applic True ("Prelude",which) [])])
  where var_list = make_var_list r
        which = case compare q r of
          LT -> "LT"
          GT -> "GT"
...
make_recur (IConstructor _ arity) var_list
  | arity == 0 = [Return (Applic True ("Prelude","EQ") [])]
  | otherwise = declare ++ [Return (merge subexprs)]
  where subexprs = [Applic False ("Prelude","compare") [Reference v, Reference w]
                      | [Variable v _, Variable w _] <- var_list]
        merge [v] = v
        merge (v1:v2:vs) = Applic False ("Prelude","compare_conjunction") [v1, merge (v2:vs)]
