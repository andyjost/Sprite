...
class TestLLVMTypes(cytest.TestCase):
  ...
  def test_isa_checks(self):
    ...
    self.assertTrue(i32.p.is_pointer)
    self.assertTrue(struct([i8]).is_struct)
    self.assertTrue((32*i8).is_vector)
    self.assertTrue(void.is_void)
    #
    self.assertFalse(i32.is_array)
    self.assertFalse(i32.is_floating_point)
    self.assertFalse(void.is_function)
    self.assertFalse(fp32.p.is_integer)
    self.assertFalse(i32.is_pointer)
