...
class TestExpr(cytest.TestCase):
  ...
  def test_choice(self):
    e = curry.expr(_choice(1, True, False))
    self.assertEqual(str(e), '_Choice 1 True False')
    self.assertEqual(repr(e), '<_Choice 1 <True> <False>>')
    self.assertEqual(sorted(curry.eval(e)), sorted([True, False]))
  ...
  def test_named_anchor1(self):
    ...
    prelude = curry.import_('Prelude')
    anchor, cons, ref = curry.anchor, curry.cons, curry.ref
