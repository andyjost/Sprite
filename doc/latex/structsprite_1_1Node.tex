...
Every node has a fixed region consisting of recount, tag, id, and arity, and a variable region called the payload. This class defines only the fixed region. A complete node is an instance of Node\_\-$<$Payload$>$ for some Payload type. The node is a discriminated union, where the tag member determines the type of the payload contents. In certain cases, the arity is also used to determine the payload type. The possible payload types are defined in the payloads namespace.

For constructor and operation nodes, the id member identifies which constructor or operation is used. The id is an index into a corresponding table in the program. That index can be used to get information about the program entity, such as its name (label) or implementation (H) function. For choice nodes, the id is the choice identifier.
